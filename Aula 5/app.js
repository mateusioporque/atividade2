...
for(let G = 0; G< cities.length; G++){
    upperCaseCities.push(cities[G].toLocaleUpperCase());
}
...
let sumNumbers = 0
for(let wq = 0; wq < arrays.length; wq++){
    for(let sa = 0; sa< arrays[wq].length;sa++){
        sumNumbers = sumNumbers + arrays[wq][sa]
    }
}
